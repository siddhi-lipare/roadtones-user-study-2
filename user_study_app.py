...
def connect_to_gsheet():
    ...
    try:
        ...
        # Keep error message for actual connection failures
...
def save_response(email, age, gender, video_data, caption_data, choice, study_phase, question_text, was_correct=None):
    ...
    # Removed info message before connection attempt
    worksheet = connect_to_gsheet() # Error is handled inside if it fails

    if worksheet:
        try:
            # Removed info message before append
            ...
            # Keep error message for append failures
...
def render_comprehension_quiz(sample, view_state_key, proceed_step):
    ...
    if st.session_state[view_state_key]['comp_feedback']:
        ...
        unique_key = f"proceed_to_captions_{sample.get('sample_id', 'unknown')}_{sample.get('video_id', 'unknown_vid')}" # Added video_id for more uniqueness
        if st.button("Proceed to Caption(s)", key=unique_key):
            ...
            st.rerun()
        streamlit_js_eval(js_expressions=JS_ANIMATION_RESET, key=f"anim_reset_comp_{unique_key}") # Use unique key

    else:
        form_key = f"comp_quiz_form_{sample.get('sample_id', 'unknown')}_{sample.get('video_id', 'unknown_vid')}" # Added video_id
        with st.form(key=form_key):
            radio_key = f"comp_radio_{sample.get('sample_id', 'unknown')}_{sample.get('video_id', 'unknown_vid')}" # Added video_id
...
# ... (Demographics, Intro Video, What is Tone, Factual Info pages remain the same) ...
    ...
    if not st.session_state.get(timer_finished_key, False) and not is_second_quality_question and not is_part2_first_question:
        ...
        if view_state_key not in st.session_state:
            ...
            if is_part2_first_question: initial_step = 5
        ...
        with col1:
            if not is_second_quality_question and not is_part2_first_question and current_step < 5: st.subheader("Watch the video")
            ...
            if current_step >= 2 and "video_summary" in sample:
                ...
                if st.session_state.get(summary_typed_key, False) or is_part2_first_question:
                    ...
                    if is_part2_first_question: st.session_state[summary_typed_key] = True
                else:
        ...
        with col2:
            ...
            if current_step >= 6:
                ...
                if "Tone Controllability" in current_part_key:
                    trait = sample['tone_to_compare']; change_type = sample['comparison_type']
                    ...
                    raw_text = question_data["question_text"]; app_trait = sample.get("application")
                    if app_trait:
                        terms_to_define.add(app_trait)
                        question_text_display = raw_text.replace(app_trait, f"<b class='highlight-trait'>{app_trait}</b>") if app_trait in raw_text else raw_text
                    ...
                    elif category_text == "writing style": question_text_display = "What is the most dominant writing style in the caption?"
                ...
                if st.session_state.show_feedback:
                    ...
                    for opt in question_data['options']:
                        is_correct = opt in correct_answer; is_user_choice = opt in user_choice
                        css_class = "correct-answer" if is_correct else ("wrong-answer" if is_user_choice else "normal-answer")
                        display_text = f"<strong>{opt} (Correct Answer)</strong>" if is_correct else (f"{opt} (Your selection)" if is_user_choice else opt)
                        st.markdown(f'<div class="feedback-option {css_class}">{display_text}</div>', unsafe_allow_html=True)
                    ...
                    if "Caption Quality" in current_part_key:
                        ...
                        if is_last_part and is_last_sample_in_part and is_last_sub_question: is_last_question = True
                    else:
                        if is_last_part and is_last_sample_in_part: is_last_question = True
                    button_text = "Finish Quiz" if is_last_question else "Next Question"

                    st.button(button_text, key=f"quiz_next_q_{sample_id}_{st.session_state.current_rating_question_index}", on_click=handle_next_quiz_question, args=(view_state_key,))
                    streamlit_js_eval(js_expressions=JS_ANIMATION_RESET, key=f"anim_reset_quiz_next_{sample_id}_{st.session_state.current_rating_question_index}") # Unique key
                    ...
                    with st.form(form_key):
                        choice = None; radio_key = f"radio_{sample_id}_{st.session_state.current_rating_question_index if 'Caption Quality' in current_part_key else ''}"
...
# ... (Quiz Results page remains the same) ...
        ...
        with st.expander("Jump to Item", expanded=True):
            if st.session_state.study_part == 1:
                ...
                for i, video in enumerate(all_videos):
                    st.button(f"`{video['video_id']}`", key=f"jump_vid_{video['video_id']}", use_container_width=True, on_click=jump_to_study_item, args=(1, i))
            elif st.session_state.study_part == 2:
                all_changes = st.session_state.all_data['study']['part2_intensity_change']
                for i, change in enumerate(all_changes):
                    st.button(f"`{change['change_id']}`", key=f"jump_chg_{change['change_id']}", use_container_width=True, on_click=jump_to_study_item, args=(2, i))
                ...
                for i, comp in enumerate(all_comparisons):
                    st.button(f"`{comp['comparison_id']}`", key=f"jump_comp_{comp['comparison_id']}", use_container_width=True, on_click=jump_to_study_item, args=(3, i))

    # --- Part 1: Caption Rating ---
    if st.session_state.study_part == 1:
        all_videos = st.session_state.all_data['study']['part1_ratings']
        ...
        current_video = all_videos[video_idx]; video_id = current_video['video_id']
        ...
        if not st.session_state.get(timer_finished_key, False) and caption_idx == 0 and not has_been_watched:
            ...
            with st.spinner(""):
                ...
                with main_col:
                    if current_video.get("orientation") == "portrait": _, vid_col, _ = st.columns([1, 3, 1]); vid_col.video(current_video['video_path'], autoplay=True, muted=True)
                    else: st.video(current_video['video_path'], autoplay=True, muted=True)
                    duration = current_video.get('duration', 10); time.sleep(duration)
            st.session_state[timer_finished_key] = True; st.rerun()
        else:
            current_caption = current_video['captions'][caption_idx]
            view_state_key = f"view_state_p1_{current_caption['caption_id']}"; summary_typed_key = f"summary_typed_{video_id}"
            ...
            if view_state_key not in st.session_state:
                initial_step = 5 if caption_idx > 0 else (5 if has_been_watched else 1)
                ...
                if caption_idx == 0: st.session_state[summary_typed_key] = has_been_watched
                ...
                if caption_idx == 0:
                    if current_step == 1:
                        ...
                        if st.session_state.get(summary_typed_key, False): st.info(current_video["video_summary"])
                            ...
                            st.session_state[summary_typed_key] = True
                        if current_step == 2 and st.button("Proceed to Question", key=f"p1_proceed_comp_q_{video_idx}"): st.session_state[view_state_key]['step'] = 3; st.rerun()
                        if current_step == 2: streamlit_js_eval(js_expressions=JS_ANIMATION_RESET, key=f"anim_reset_study_1_2_{video_idx}")
                else: st.subheader("Video Summary"); st.info(current_video["video_summary"])
            with col2:
                ...
                if current_step >= 5:
                    ...
                    if current_step == 5 and st.button("Show Questions", key=f"show_q_{current_caption['caption_id']}"): st.session_state[view_state_key]['step'] = 6; st.rerun()
                if current_step >= 6:
                    control_scores = current_caption.get("control_scores", {})
                    tone_traits = list(control_scores.get("tone", {}).keys())[:2]; style_traits = list(control_scores.get("writing_style", {}).keys())[:2]
                    ...
                    def format_traits(traits):
                        ...
                        return " and ".join(highlighted) if len(highlighted) > 1 else (highlighted[0] if highlighted else "")
                    ...
                    found_override = False; style_q_text_final = ""; style_q_options_final = []
                    for trait in style_traits:
                        if trait in style_overrides:
                            style_q_text_final = style_overrides[trait]['text']; style_q_options_final = style_overrides[trait]['options']
                            terms_to_define.add(trait); found_override = True; break
                    if not found_override:
                        style_str = format_traits(style_traits)
                        default_text = style_q_template_obj.get('default_text', "How {} is the caption's style?") if style_q_template_obj else "How {} is the caption's style?"
                        default_options = style_q_template_obj.get('default_options', ["Not at all", "Weak", "Moderate", "Strong", "Very Strong"]) if style_q_template_obj else ["Not at all", "Weak", "Moderate", "Strong", "Very Strong"]
                        style_q_text_final = default_text.format(style_str); style_q_options_final = default_options
                    ...
                    questions_to_ask = [{"id": "tone_relevance", "text": tone_q_template.format(tone_str)}, {"id": "style_relevance", "text": style_q_text_final}, {"id": "factual_consistency", "text": fact_q_template}, {"id": "usefulness", "text": useful_q_template.format(f"<b class='highlight-trait'>{application_text}</b>")}, {"id": "human_likeness", "text": human_q_template}]
                    ...
                    def render_slider(q, col, q_index, view_key_arg):
                        with col:
                            slider_key = f"ss_{q['id']}_cap{caption_idx}_{video_id}" # Added video_id
                    ...
                    if questions_to_show > len(questions_to_ask):
                        if st.button("Submit Ratings", key=f"submit_cap{caption_idx}_{video_id}"): # Added video_id
                            all_interacted = all(interacted_state.get(qid, False) for qid in question_ids)
                            if not all_interacted: validation_placeholder.warning(f"⚠️ Please move the slider for question(s): {', '.join(map(str, [i+1 for i, qid in enumerate(question_ids) if not interacted_state.get(qid, False)]))}")
                            else:
                                with st.spinner("Saving response..."): # Added spinner here too
                                    all_saved = True
                                    responses_to_save = {qid: st.session_state.get(f"ss_{qid}_cap{caption_idx}_{video_id}") for qid in question_ids} # Added video_id
    ...
    # --- Part 2: Tone Intensity Change ---
        ...
        current_change = all_changes[change_idx]; change_id = current_change['change_id']; video_id = current_change.get('video_id')
        field_to_change = current_change['field_to_change']; field_type = list(field_to_change.keys())[0]
        timer_finished_key = f"timer_finished_{change_id}"; has_been_watched = video_id in st.session_state.comprehension_passed_video_ids
        ...
        if not st.session_state.get(timer_finished_key, False) and not has_been_watched:
            ...
            with st.spinner(""):
                main_col, _ = st.columns([1, 1.8])
                with main_col:
                    if current_change.get("orientation") == "portrait": _, vid_col, _ = st.columns([1, 3, 1]); vid_col.video(current_change['video_path'], autoplay=True, muted=True)
            ...
            if view_state_key not in st.session_state:
                initial_step = 5 if has_been_watched else 1
                st.session_state[view_state_key] = {'step': initial_step, 'summary_typed': has_been_watched, 'comp_feedback': False, 'comp_choice': None}
                if not has_been_watched: st.session_state[summary_typed_key] = False
            current_step = st.session_state[view_state_key]['step']

            title_col1, title_col2 = st.columns([1, 1.8])
            with title_col1: st.subheader("Video")
            with title_col2:
                if current_step >= 5: st.subheader(f"Tone Intensity Change")
            ...
            with col1:
                ...
                if current_step == 1:
                    if st.button("Proceed to Summary", key=f"p2_proceed_summary_{change_id}"): st.session_state[view_state_key]['step'] = 2; st.rerun()
                ...
                if current_step >= 2:
                    ...
                    if st.session_state.get(summary_typed_key, False): st.info(current_change["video_summary"])
                        ...
                        st.session_state[summary_typed_key] = True
                    if current_step == 2 and st.button("Proceed to Question", key=f"p2_proceed_captions_{change_id}"): st.session_state[view_state_key]['step'] = 3; st.rerun()
                    if current_step == 2: streamlit_js_eval(js_expressions=JS_ANIMATION_RESET, key=f"anim_reset_study_2_2_{change_id}")
            with col2:
                if current_step == 3 or current_step == 4: render_comprehension_quiz(current_change, view_state_key, proceed_step=5)
                if current_step >= 5:
                    ...
                    if current_step == 5 and st.button("Show Questions", key=f"p2_show_q_{change_id}"): st.session_state[view_state_key]['step'] = 6; st.rerun()
                if current_step >= 6:
                    terms_to_define = set(); trait = field_to_change[field_type]; terms_to_define.add(trait)
                    q_template = None; q_template_key = 'Writing Style' if field_type == 'writing_style' else ('Tone' if field_type == 'tone' else None)
                    if not q_template_key: st.error(f"Unknown field type: {field_type}")
                    else:
                        if q_template_key not in st.session_state.all_data['questions']['part2_questions']: st.error(f"Template key '{q_template_key}' not found")
                        else: q_template = st.session_state.all_data['questions']['part2_questions'][q_template_key]
                    if q_template:
                        with st.form(key=f"study_form_p2_{change_idx}_{video_id}"): # Added video_id
                            ...
                            with col_q1:
                                ...
                                choice1 = st.radio("q1_label", ["Yes", "No"], index=None, horizontal=True, key=f"p2_{change_id}_q1", label_visibility="collapsed")
                            with col_q2:
                                st.markdown(f'<div class="part3-question-text">2. {q2_text}</div>', unsafe_allow_html=True)
                                choice2 = st.radio("q2_label", ["Yes", "No"], index=None, horizontal=True, key=f"p2_{change_id}_q2", label_visibility="collapsed")
    ...
    # --- Part 3: Caption Comparison ---
        ...
        current_comp = all_comparisons[comp_idx]; comparison_id = current_comp['comparison_id']; video_id = current_comp.get('video_id')
        timer_finished_key = f"timer_finished_{comparison_id}"; has_been_watched = video_id in st.session_state.comprehension_passed_video_ids
        ...
        if not st.session_state.get(timer_finished_key, False) and not has_been_watched:
            ...
            with st.spinner(""):
                main_col, _ = st.columns([1, 1.8])
                with main_col:
                    if current_comp.get("orientation") == "portrait": _, vid_col, _ = st.columns([1, 3, 1]); vid_col.video(current_comp['video_path'], autoplay=True, muted=True)
            ...
            q_templates = st.session_state.all_data['questions']['part3_questions']; question_ids = [q['id'] for q in q_templates]
            if view_state_key not in st.session_state:
                initial_step = 5 if has_been_watched else 1
                st.session_state[view_state_key] = {'step': initial_step, 'interacted': {qid: False for qid in question_ids}, 'comp_feedback': False, 'comp_choice': None}
                if not has_been_watched: st.session_state[summary_typed_key] = False
                else: st.session_state[summary_typed_key] = True # Ensure it's true if watched
            current_step = st.session_state[view_state_key]['step']

            def mark_p3_interacted(q_id, view_key):
                if view_key in st.session_state and 'interacted' in st.session_state[view_key]:
                    if not st.session_state[view_key]['interacted'][q_id]: st.session_state[view_key]['interacted'][q_id] = True
            ...
            with col1:
                ...
                if current_step == 1:
                    if st.button("Proceed to Summary", key=f"p3_proceed_summary_{comparison_id}"): st.session_state[view_state_key]['step'] = 2; st.rerun()
                ...
                if current_step >= 2:
                    ...
                    if st.session_state.get(summary_typed_key, False): st.info(current_comp["video_summary"])
                        ...
                        st.session_state[summary_typed_key] = True
                    if current_step == 2 and st.button("Proceed to Question", key=f"p3_proceed_captions_{comparison_id}"): st.session_state[view_state_key]['step'] = 3; st.rerun()
                    if current_step == 2: streamlit_js_eval(js_expressions=JS_ANIMATION_RESET, key=f"anim_reset_study_3_2_{comparison_id}")
            with col2:
                if current_step == 3 or current_step == 4: render_comprehension_quiz(current_comp, view_state_key, proceed_step=5)
                ...
                if current_step >= 5:
                    ...
                    if current_step == 5 and st.button("Show Questions", key=f"p3_show_q_{comparison_id}"): st.session_state[view_state_key]['step'] = 6; st.rerun()
                if current_step >= 6:
                    ...
                    for q in q_templates:
                        q_id = q['id']; style_q_text_final = ""
                        if q_id == 'q2_style':
                            style_overrides = q.get('overrides', {}); found_override = False
                            for trait in style_traits:
                                if trait in style_overrides:
                                    style_q_text_final = style_overrides[trait]['text']; terms_to_define.add(trait); found_override = True; break
                            if not found_override:
                        ...
                        elif q_id == 'q1_tone': part3_questions.append({"id": q_id, "text": q['text'].format(tone_str)})
                        else: part3_questions.append({"id": q_id, "text": q.get('text', '')})
                    ...
                    if questions_to_show > len(part3_questions):
                        if st.button("Submit Comparison", key=f"submit_comp_p3_{comparison_id}"):
                            ...
                            if any(choice is None for choice in responses.values()): validation_placeholder.warning("⚠️ Please answer all four questions before submitting.")
                            else:
                                with st.spinner("Saving response..."):
                                    all_saved = True
    ...
    if (event.key === 'ArrowRight') {
        ...
        for (const label of targetButtonLabels) {
            ...
            if (targetButton) {
                ...
                targetButton.click(); break;
            }
        }
    } else if (event.key === 'ArrowLeft') {
        event.preventDefault();
        const targetButtonLabels = ["Prev <<"];
        const allButtons = Array.from(parent_document.querySelectorAll('button'));
        const visibleButtons = allButtons.filter(btn => btn.offsetParent !== null);
        for (const label of targetButtonLabels) {
            const targetButton = [...visibleButtons].reverse().find(btn => btn.textContent.trim().includes(label));
...
"""
streamlit_js_eval(js_expressions=js_script, key="keyboard_listener_v5") # Incremented key again